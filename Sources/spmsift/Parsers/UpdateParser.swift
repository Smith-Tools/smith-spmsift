...
public struct UpdateParser {
    ...
    public func parse(_ input: String, targetFilter: String? = nil) throws -> PackageAnalysis {
        // Note: update command doesn't provide target-specific information
        ...
        if targetFilter != nil {
            return PackageAnalysis(
                ...
                issues: [PackageIssue(
                    ...
                    severity: .info,
                    target: targetFilter,
                    message: "update command doesn't support target-specific analysis. Use dump-package for target filtering."
                )]
            )
        }
